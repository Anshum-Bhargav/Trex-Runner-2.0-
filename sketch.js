...
function draw() {
  ...
  if (gameState===PLAY){
    ...
    if(touches.width>0 && touches.height>0 || mousePressedOver(restart)) {
      reset();
    }
  }
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    ...
    cloud.y = Math.round(random(height-100,height-800));
...
function spawnObstacles() {
  if(frameCount % 60 === 0) {
    ...
    //spawning Obstacles in Different x positions
    //obstacle.x = Math.round(random(width,width+300));
